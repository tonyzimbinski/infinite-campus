...
function buildSubNav (obj) {
  ...
  // edited: now all methods are shown no matter what
  ...
  if (items.length) {
    ...
    items.forEach(function (item) {
      ...
      if (!hasOwnProp.call(item, 'longname')) {
        ...
        // edited: added id "<name>_navTag" to <a> tags
        //        linkHtml = linktoFn(item.longname, displayName.replace(/\b(module|event):/g, ''))
        linkHtml = linktoFn(item.longname, displayName.replace(/\b(module|event):/g, '')).replace('href="', 'id="' + item.longname + '_navTag" href="')
