...
class User extends EventEmitter {
  ...
  */
    ...
    constructor(districtName, state, username, password) {
        ...
        this._getDistrict(districtName, state)
            .then(() => {
            ...
            .catch((err) => {
                ...
                throw Error(
                    "Unknown error during the getDistrict/login process. If you keep getting this error please submit an issue on GitHub @" +
                        meta.github +
                        "\n " +
                        err
                );
  ...
  */
    _getDistrict(districtName, state) {
        return new Promise((resolve, reject) => {
            request(
                "https://mobile.infinitecampus.com/mobile/searchDistrict?query=" +
                    districtName +
                    "&state=" +
                    state,
                (err, res, body) => {
    ...
    _login(username, password) {
        return new Promise((resolve, reject) => {
            if (this.district === undefined) {
                throw Error(
                    "._getDistrict() must be ran before you can log in."
                );
            }
                        ...
                        (err, res, body) => {
                            ...
                            if (grades.length > 1) {
                                ...
                                if (schoolID === undefined) {
                                    console.warn(
                                        `WARNING: You are enrolled in ${grades.length} schools, please explicitly specify a school ID to fetch courses from. Please see the below output to see which schoolID to use. (Defaulting to the first school returned by I.C. API - name: '${schools[0].schoolName}' - id: ${schools[0].id})`,
                                        schools
                                    );
    ...
    getNotificationCount() {
        return new Promise((resolve, reject) => {
            ...
            request(
                this.district.district_baseurl +
                    "prism?x=notifications.NotificationUser-countUnviewed",
                (err, res, body) => {
                    errHandler.generic200.handle(err, res, body);
                    resolve(
                        parseInt(
                            JSON.parse(body).data.RecentNotifications.count
                        )
                    );
                }
            );
    ...
    markAllNotificationsRead() {
        return new Promise((resolve, reject) => {
            ...
            request(
                this.district.district_baseurl +
                    "prism?x=notifications.Notification-markAllRead",
                (err, res, body) => {
                    errHandler.generic200.handle(err, res, body);
                    resolve();
                }
                    ...
                    body.forEach((notification) => {
                        result.push({
                            ...
                            timestamp:
                                ...
                                ).getTime() / 1000, // convert text to unix ms to s
...
function checkAuth() {
    ...
    if (!this.authenticated || this.district === undefined) {
        throw Error(
            "User isn't authenticated yet. Please use the .on('ready') event emitted by the User class. For help please see the example code on GitHub: https://github.com/" +
                meta.github
        );
    }
}
